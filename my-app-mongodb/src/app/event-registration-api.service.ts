...
export class EventRegistrationApiService {
 private apiUrl = 'http://localhost:3000/eventregistrations';
  constructor(private http: HttpClient) { }

  getEventRegistration(page: number, pageSize: number, search?: string, sortBy?: string, sortOrder: 'asc' | 'desc' = 'asc'): Observable<{ data: EventRegistration[]; total: number }> {
    ...
    return this.http.get<{ data: EventRegistration[]; total: number }>(this.apiUrl, { params }).pipe(
      retry(3),
